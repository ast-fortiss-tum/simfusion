...
def wandb_log_artifact(
    ...
    try:
        ...
        if artifact_filename is None:
            ...
            artifact.add_file(file_path, name=artifact_filename)

        wandb.log_artifact(artifact)
        print(f"Successfully logged artifact '{artifact_name}'")

    except Exception:
        print(f"Failed to log artifact '{artifact_name}':")
        traceback.print_exc()

def wandb_log_folder(
    folder_path: str,
    artifact_name: str,
    artifact_type: str = "output",
    ...
    try:
        ...
        for root, dirs, files in os.walk(folder_path):
            ...
            # ---- Exclude directories (prevents traversal) ----
            dirs[:] = [
                d for d in dirs
                if not any(
                    fnmatch.fnmatch(
                        os.path.join(rel_root, d), pattern
                    )
            ...
            for file in files:
                ...
                artifact.add_file(full_path, name=relative_path)

        wandb.log_artifact(artifact)
        print(f"Successfully logged folder '{artifact_name}'")

    except Exception:
        print(f"Failed to log folder '{artifact_name}':")
        traceback.print_exc()
        
def wandb_log_metrics_df(df, prefix=""):
    """
    Logs metrics from a DataFrame to W&B.
    Assumes the DataFrame has columns: 'metric' and 'count'.
    """
    if df.empty:
        print("DataFrame is empty. Nothing to log.")
        return
    
    metrics_dict = {}
    for _, row in df.iterrows():
        metric_name = prefix + row['metric']
        metric_value = row['count']
        # Convert numeric values to float for logging
        try:
...
def wandb_log_metrics_from_csv(filename):
    # Only log if file exists and is not empty
    if (
