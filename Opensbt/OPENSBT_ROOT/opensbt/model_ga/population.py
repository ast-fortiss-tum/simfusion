...
class PopulationExtended(Population):
    ...
    def divide_critical_non_critical(self):
        ...
        # Count critical individuals explicitly
        # print("get CB values:", [ind.get("CB") for ind in self])
        n_crit = sum(1 for ind in self if ind.get("CB") == 1)
        ...
        for ind in self:
            if ind.get("CB") == 1:
                critical_population[i_crit] = deepcopy(ind)
                i_crit += 1
            else:
                notcritical_population[i_ncrit] = deepcopy(ind)
                i_ncrit += 1
