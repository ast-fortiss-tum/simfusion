...
class SimulationResult(Result):
    ...
    def write_results(self, 
                     ...
                     save_folder = None,
                     upload_wandb = False):  # we can declare the results folder, or the save folder (specific for experiment)
        ...
        if hasattr(self.problem, 'predictor'):
            ...
            dst = os.path.join(save_folder, "predictor")
        ...
        ### lofi hifi specific
        if "MultiSim" in type(self.problem).__name__ or \
            "ADASProblemSurrogate" in type(self.problem).__name__ :
            if WRITE_ALL_INDIVIDUALS:
                visualizer.all_individuals_lohifi(self, save_folder)
            
            if type(self.problem).__name__ == "ADASMultiSimPredictProblem" or \
                type(self.problem).__name__ == "ADASMultiSimPredictCertainProblem":
                visualizer.all_individuals_lohifi_predictions(self, save_folder)

                try:
                    if type(self.problem).__name__ == "ADASMultiSimPredictCertainProblem":
                                visualizer.all_individuals_lohifi_predictions_certainty(self, save_folder)
                except Exception as e:
                    log.warning(f"Could not write certainty values based output: {e}")
                    pass

            visualizer.write_generations_lohifi(self, save_folder)
            visualizer.simulations(self, 
                save_folder,
                mode = config.MODE_WRITE_GIF,
                write_max = config.NUM_GIF_MAX,
                accessor="SO_LOFI",
                name_folder="gif_lofi")
            ...
            visualizer.plot_timeseries_basic(self,
                                ...
                                mode= config.MODE_PLOT_TIME_TRACES,
                                    ...
                                    accessor="SO_LOFI",
                                    name_folder="trace_comparison_lofi")
        ...
        output_metric.hypervolume_analysis(self, save_folder)
        visualizer.write_diversity(self.obtain_archive(), 
