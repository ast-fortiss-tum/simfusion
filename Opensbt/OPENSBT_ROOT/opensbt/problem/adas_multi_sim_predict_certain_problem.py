...
@dataclass
class ADASMultiSimPredictCertainProblem(Problem):
    ...
    def __init__(self,
        ...
        :param predictor: The agreement predictor instance, defaults to None
        :type predictor: AgreementPredictor, optional
        :param certainty_threshold: Minimum certainty threshold to run LoFi (default 1.0 = 100%)
        :type certainty_threshold: float, optional
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        out["C"] = [None] * n  # Certainty output

        for i, xi in enumerate(x):
            log.info(f"Evaluating xi: {xi}")
            
            # Predictor decides which fidelity to execute with certainty
            try:
                prediction, certainty = self.predictor.predict_with_certainty(np.asarray([xi]))
            except Exception as e:
                log.info("Exception in predictor occurred:")
                raise e

            out["P"][i] = int(prediction)  # Save prediction
            out["C"][i] = float(certainty)  # Save certainty
            
            log.info(f"Prediction: {prediction} ({'Agreement' if prediction == 1 else 'Disagreement'})")
            log.info(f"Certainty: {certainty:.2%}")

            assert prediction in [0, 1]
            
            # Decision logic: Run LoFi if agreement (1) AND certainty >= threshold
            run_lofi = (prediction == 1) and (certainty >= self.certainty_threshold)
            
            if run_lofi: 
                log.info(f"Running LoFi simulation (Agreement with certainty {certainty:.2%} >= {self.certainty_threshold:.2%})")
                ...
                log.info(f"LoFi critical: {is_critical_lofi}")

                # Use LoFi results as final output
                out["SO"][i] = simout_lofi
                out["F"][i] = fitness_lofi
                out["CB"][i] = is_critical_lofi

            else: 
                # Run HiFi simulation (disagreement OR low certainty)
                reason = "Disagreement" if prediction == 0 else f"Low certainty ({certainty:.2%} < {self.certainty_threshold:.2%})"
                log.info(f"Running HiFi simulation ({reason})")
                ...
                log.info(f"HiFi critical: {is_critical_hifi}")

                # Use HiFi results as final output
        ...
        log.info(f"F: {out['F']}")
        log.info(f"Predictions: {out['P']}")
        log.info(f"Certainties: {out['C']}")
