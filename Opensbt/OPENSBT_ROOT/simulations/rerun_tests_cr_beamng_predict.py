...
def rerun_and_analyze(folder: str, sim_rerun: str):
    ...
    pop_all = copy.deepcopy(pop_all) 

    pop_validation = []
    for ind in pop_all:
        f_hifi = ind.get("F_HIFI")
        if f_hifi is None or all(v is None for v in f_hifi):
            pop_validation.append(ind)
