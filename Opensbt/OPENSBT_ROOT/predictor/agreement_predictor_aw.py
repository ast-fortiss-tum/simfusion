...
class AgreementPredictorAutoware:
    def __init__(
        self,
        csv_folder: str,
    ...
    def plot_certainty_distributions(self, certainty_data: Dict, save_path: Optional[str] = None):
        ...
        fig.suptitle(f'Certainty Distributions - {self.model_type.upper()}', fontsize=16, fontweight='bold')
...
def report_best_models_pareto(
    *,
    ...
    cols_to_show = ["batch", "model_type"] + metrics + ["best_search_score", "best_params"]
...
if __name__ == "__main__":
    # Example 1: Training and evaluation workflow with grid search
    batches = ["data_final"]
    ...
    for batch_path in batches:
        csv_folder = f"./predictor/data_aw/{batch_path}/"
        features = [
            "PedSpeed",
            "EgoSpeed",
            "PedDist"
        ...
        metrics_out_dir = f"./results/{batch_path}/"
        ...
        for model_type, model_params in model_configs:
            ...
            predictor = AgreementPredictorAutoware(
            ...
            predictor.plot_certainty_distributions(
                certainty_data, 
                save_path=f"{metrics_out_dir}certainty_{model_type}.png"
            )
            ...
            if run_grid_search and model_type in param_grids:
                ...
                cfg_path = export_best_model_config(
                    out_dir=metrics_out_dir,
                    filename=f"best_model_config_{model_type}.json",
                    batch=batch_path,
                    model_type=model_type,
                    best_params=gs_out["best_params"],
    ...
    # # Example 2: Inference with uncertainty quantification
    # print("\n\n" + "#" * 80)
    # print("INFERENCE EXAMPLE WITH UNCERTAINTY")
    # print("#" * 80)
    
    # # Load a trained model (or use the one from above)
    ...
    # # Test samples
    # test_samples = [
    #     [150.0, 0.0, 10, 20, 1, 1, 9, 15, 0, 0.5, 0],
    #     [153.0, 0.0, 12, 21, 1, 1, 10, 16, 0, 0.5, 0],
    #     [154.0, 0.0, 11, 22, 1, 1, 10, 17, 0, 0.5, 0]
    # ]
    
    # print("\nPredictions with certainty:")
    # for sample in test_samples:
    #     x_test = np.array([sample])
    #     label, certainty = predictor.predict_with_label_and_certainty(x_test)
    #     print(f"Input: {sample}")
    #     print(f"  → {label} (certainty: {certainty:.2%})")
    
    # print("\nDetailed prediction for first sample:")
    # details = predictor.get_prediction_details(np.array([test_samples[0]]))
    # print(f"Prediction: {details['label']}")
    # print(f"Certainty: {details['certainty']:.2%}")
    # print(f"Probabilities: {details['probabilities']}")
    # if 'neighbors' in details:
    #     print(f"Neighbors: k={details['neighbors']['k']}, "
    #           f"Agreement={details['neighbors']['agreement_count']}, "
    #           f"Disagreement={details['neighbors']['disagreement_count']}")

    # # Example 3: Load from config file for inference
    # # predictor = AgreementPredictor(
    # #     csv_folder="./predictor/data_cr_beamng/data_final/",
    # #     feature_columns=features,
    # #     config_path="./results/data_final/best_model_config_rf.json",
    # #     use_class_weights=True,
    # # )