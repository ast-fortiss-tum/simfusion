...
def plot_design_space(input_path: str, save_folder: str, name_suffix: str = ""):
    ...
    for cat, color in colors.items():
        ...
        ax.scatter(subset['goal_center_x'], subset['goal_center_y'], subset['goal_velocity_max'],
                   label=cat, c=color, s=50, edgecolors='k')
    ax.set_title(format_title_with_counts("Design Space (3D)", category_counts))
    ax.set_xlabel("goal_center_x")
    ax.set_ylabel("goal_center_y")
    ax.set_zlabel("goal_velocity_max")
    ax.legend()
    plt.tight_layout()
    ...
    # 2D Projections
    projections = [
        ("goal_center_x", "goal_center_y", f"design_space_goal_x_vs_goal_y{name_suffix}.png", "Top-Down View"),
        ("goal_center_y", "goal_velocity_max", f"design_space_goal_y_vs_goal_velocity_max{name_suffix}.png", "Side View 1"),
        ("goal_center_x", "goal_velocity_max", f"design_space_goal_x_vs_goal_velocity_max{name_suffix}.png", "Side View 2")
    ]
...
def plot_objective_space(input_path: str, save_folder: str, name_suffix: str = ""):
    ...
    df_lofi = df[(df['Fitness_Min distance to obstacle_LoFi'] != 1000.0) & (df['Fitness_Distance to goal_LoFi'] != 1000.0)]
    category_counts_lofi = df_lofi['Category'].value_counts().to_dict()
    fig, ax = plt.subplots(figsize=(8, 6))
    for cat, color in colors.items():
        subset = df_lofi[df_lofi['Category'] == cat]
        ax.scatter(subset['Fitness_Min distance to obstacle_LoFi'], 
                   subset['Fitness_Distance to goal_LoFi'],
                   label=cat, c=color, s=50, edgecolors='k')
    ax.set_title(format_title_with_counts("Objective Space (LoFi)", category_counts_lofi))
    ax.set_xlabel("Fitness_Min distance to obstacle_LoFi")
    ax.set_ylabel("Fitness_Distance to goal_LoFi")
    ax.legend()
    plt.tight_layout()
    ...
    df_hifi = df[(df['Fitness_Min distance to obstacle_HiFi'] != 1000.0) & (df['Fitness_Distance to goal_HiFi'] != 1000.0)]
    category_counts_hifi = df_hifi['Category'].value_counts().to_dict()
    fig, ax = plt.subplots(figsize=(8, 6))
    for cat, color in colors.items():
        subset = df_hifi[df_hifi['Category'] == cat]
        ax.scatter(subset['Fitness_Min distance to obstacle_HiFi'], subset['Fitness_Distance to goal_HiFi'],
                   label=cat, c=color, s=50, edgecolors='k')
    ax.set_title(format_title_with_counts("Objective Space (HiFi)", category_counts_hifi))
    ax.set_xlabel("Fitness_Min distance to obstacle_HiFi")
    ax.set_ylabel("Fitness_Distance to goal_HiFi")
    ax.legend()
    plt.tight_layout()
