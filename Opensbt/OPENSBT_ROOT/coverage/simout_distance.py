...
class UtterancesDistance(BaseModel):
    ...
    def calculate(
        ...
        if a is None or b is None or not getattr(a, "question") or not getattr(b, "question"):
            q_similarity = 0
        else:
        ...
        if a is None or b is None or not getattr(a, "answer") or not getattr(b, "answer"):
            a_similarity = 0
        else:
