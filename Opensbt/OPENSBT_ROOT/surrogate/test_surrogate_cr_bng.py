...
# === CONFIGURATION ===
input_columns = ["goal_center_x","goal_center_y","goal_velocity_min","goal_velocity_max","num_npc","lane_count","npc_x0","npc_v_max","npc_seed","cutin_tx","cutin_delay"]
optional_columns =  ["Fitness_Min distance to obstacle","Fitness_Distance to goal"]
desired_columns = ["Fitness_Min distance to obstacle_HiFi","Fitness_Distance to goal_HiFi"]
output_folder = "./surrogate/plots/"
model_output_path = "./surrogate/out/model_"
save_folder = "./surrogate/out/"
scenario_file = "CUT_IN_last.xml"
...
def compute_criticality(df, target_cols):
...
def compute_criticality_points(F, critical_function):
    ...
    critical_tests = [t for t in F if critical_function.eval(t)]
    print(f"num tests (training): {len(F)}")
    print(f"num critical: {len(critical_tests)}")
    print(f"non critical tests: {len(F) - len(critical_tests)}")
    return label_values
...
def apply_smote_with_fit(X, y_reg, y_class, apply_smote, data_folder_name,
    ...
    if gt_fit:
        problem = ADASProblem(
            ...
            xl=xl,
            xu=xu,
            simulation_variables=input_columns,
...
def train_model(model_name, data_folder, 
    ...
    X_resampled, y_reg_resampled = \
        ...
        X, y_reg, y_class, apply_smote, data_folder_name,
        gt_fit=gt_fit,
    ...
    F_all = np.concatenate([y_train, y_val], axis=0)
    y_labels = compute_criticality_points(F_all, critical_function)
    print(y_labels)
